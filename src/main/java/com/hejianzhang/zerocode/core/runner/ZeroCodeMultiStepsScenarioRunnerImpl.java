...
import com.hejianzhang.zerocode.core.engine.preprocessor.ScenarioExecutionState;
import com.hejianzhang.zerocode.core.engine.preprocessor.StepExecutionState;
...
import com.hejianzhang.zerocode.core.runner.StepNotificationHandler;
...
import static com.hejianzhang.zerocode.core.domain.ZerocodeConstants.*;
import static com.hejianzhang.zerocode.core.domain.builders.ZeroCodeExecResultBuilder.newInstance;
import static com.hejianzhang.zerocode.core.engine.mocker.RestEndPointMocker.wireMockServer;
import static com.hejianzhang.zerocode.core.utils.RunnerUtils.getFullyQualifiedUrl;
import static com.hejianzhang.zerocode.core.utils.ServiceTypeUtils.serviceType;
import static com.hejianzhang.zerocode.core.utils.SmartUtils.prettyPrintJson;
...
public class ZeroCodeMultiStepsScenarioRunnerImpl implements ZeroCodeMultiStepsScenarioRunner {
    ...
    private static final Logger LOGGER = getLogger(com.hejianzhang.zerocode.core.runner.ZeroCodeMultiStepsScenarioRunnerImpl.class);
    ...
    public synchronized boolean runScenario(ScenarioSpec scenario, RunNotifier notifier, Description description) {
        ...
        regresscasesResultMapper regresscases = (regresscasesResultMapper)wac.getBean("regresscasesResultMapper");
        ...
        for (int k = 0; k < scenarioLoopTimes; k++) {
            ...
            for (Step thisStep : scenario.getSteps()) {
                ...
                for (int i = 0; i < stepLoopTimes; i++) {
                    ...
                    regresscasesResult  regresscasesResult = regresscases.selectByPrimaryKey(new Integer(thisStep.getId()));
                    ...
                    try {
                        ...
                        if (!failureResults.isEmpty()) {
                            ...
                            regresscasesResult.setUpdatetime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                            regresscasesResult.setResult(builder.toString());
                            regresscases.updateByPrimaryKey(regresscasesResult);
                        } else {
                            logCorrelationshipPrinter.assertion(prettyPrintJson(resolvedAssertionJson));
                            regresscasesResult.setUpdatetime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                            regresscasesResult.setResult("success");
                            regresscases.updateByPrimaryKey(regresscasesResult);
                        }
                        // --------------------------------------------------------------------------------
                        ...
                        regresscasesResult.setUpdatetime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                        regresscasesResult.setResult(ex.getMessage());
                        regresscases.updateByPrimaryKey(regresscasesResult);
